...
export function UploadModal({ isOpen, onClose, onFileSelect }: UploadModalProps) {
  ...
  const getFormatIcon = (filename: string) => {
    ...
    switch (label) {
      case 'XLSX': return <Sheet className="w-5 h-5 text-[#D4A847]" />;
      case 'XLS': return <Table2 className="w-5 h-5 text-amber-600" />;
      case 'TSV': return <FileText className="w-5 h-5 text-[#94A3B8]" />;
  ...
  const getFormatColor = (filename: string) => {
    const label = getFileFormatLabel(filename);
    switch (label) {
      case 'XLSX': return 'bg-[#1E2D45] text-[#A07A28]';
      case 'XLS': return 'bg-amber-100 text-amber-700';
      case 'TSV': return 'bg-[#1C2537] text-[#F0EDE6]';
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className="relative w-full max-w-lg bg-[#111827] rounded-3xl shadow-2xl animate-[modalSlideUp_0.3s_ease-out] overflow-hidden"
      >
        {/* Header */}
        <div className="relative px-6 pt-6 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#F0EDE6]">Upload New Report</h2>
                <p className="text-xs text-[#475569]">Replace current data with a new file</p>
              </div>
            </div>
            <button
              onClick={onClose}
              className="w-9 h-9 rounded-xl flex items-center justify-center text-[#475569] hover:text-[#94A3B8] hover:bg-[#1C2537] transition-all duration-200"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="px-6 pb-2">
          <div
            ...
            className={`
              ...
              ${isDragOver
                ? 'border-primary-400 bg-[#1C2537] scale-[1.02]'
                : selectedFile
                  ? 'border-emerald-300 bg-emerald-50/50'
                  : 'border-[#1E2D45] bg-[#0B0F1A]/50 hover:border-primary-300 hover:bg-[#1C2537]/30'
              }
            `}
          >
            {selectedFile ? (
              <>
                ...
                <p className="text-sm font-semibold text-[#F0EDE6] mb-1 truncate max-w-xs mx-auto">{selectedFile.name}</p>
                <div className="flex items-center justify-center gap-2 mt-2">
                  ...
                  <span className="text-xs text-[#475569]">{formatFileSize(selectedFile.size)}</span>
                </div>
                <p className="text-xs text-[#475569] mt-2">Click to change file</p>
              </>
            ) : (
              <>
                <div className={`
                  w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-4 transition-all duration-300
                  ${isDragOver ? 'bg-[#1E2D45] scale-110' : 'bg-[#1C2537] group-hover:bg-[#1E2D45] group-hover:scale-105'}
                `}>
                  <Upload className={`w-7 h-7 transition-colors duration-300 ${isDragOver ? 'text-[#D4A847]' : 'text-[#475569] group-hover:text-primary-500'}`} />
                </div>
                <p className="text-sm font-semibold text-[#F0EDE6] mb-1">
                  Drop your file here
                </p>
                <p className="text-xs text-[#475569]">or click to browse • Max 50MB</p>
              </>
            )}
          </div>
        ...
        <div className="px-6 pt-5 pb-4 flex items-center gap-3">
          <button
            onClick={onClose}
            className="flex-1 px-5 py-3 rounded-xl border border-[#1E2D45] text-sm font-medium text-[#94A3B8] hover:bg-[#0B0F1A] hover:border-[#1E2D45] transition-all duration-200"
          >
            Cancel
          </button>
          <button
            ...
            className={`
              ...
              ${selectedFile
                ? 'bg-gradient-to-r from-[#D4A847] to-[#A07A28] text-white shadow-md shadow-primary-200 hover:shadow-lg hover:shadow-primary-300 hover:-translate-y-0.5'
                : 'bg-[#1C2537] text-[#475569] cursor-not-allowed'
              }
            `}
          >
        ...
        <div className="px-6 pb-5">
          <div className="flex items-center justify-center gap-3 flex-wrap">
            {[
              ...
              <span key={f.ext} className="flex items-center gap-1.5 text-[11px] text-[#475569]">
