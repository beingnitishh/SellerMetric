...
export function Dashboard({ data, onReset, onFileSelect, returnData, onUploadReturn, onClearReturn, initialTab = 'sales' }: DashboardProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 gradient-mesh">
      {/* Top Bar */}
      <div className="sticky top-0 z-50 glass-card border-b border-slate-200/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <button
            onClick={onReset}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-slate-600 hover:text-primary-600 hover:bg-primary-50 transition-all duration-200"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-md shadow-primary-200 hover:shadow-lg hover:shadow-primary-300 hover:-translate-y-0.5 transition-all duration-200"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        {/* Tab Navigation */}
        <div className="flex items-center gap-1 bg-white rounded-2xl p-1.5 border border-slate-200 shadow-sm max-w-md">
          <button
          ...
          >
            ...
            {returnData.length > 0 && (
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                activeTab === 'returns' ? 'bg-white/25 text-white' : 'bg-amber-100 text-amber-700'
              }`}>
                {returnData.length}
              </span>
            )}
        ...
        {activeTab === 'sales' && (
          <>
            ...
            {!data ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-300 animate-fade-in-up">
                ...
                <h2 className="text-2xl font-bold text-slate-800 mb-2">No Sales Data Loaded</h2>
                ...
                <button
                  onClick={() => setIsUploadModalOpen(true)}
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-lg shadow-primary-200 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200"
                >
                  <Upload className="w-4 h-4" />
                  Upload Sales Report
                </button>
                    ...
                    <table className="min-w-full divide-y divide-slate-100 text-sm">
                      <thead>
                        <tr className="bg-slate-50/80">
                          <th className="px-6 py-3.5 text-left font-semibold text-slate-600 text-xs uppercase tracking-wider">Metric</th>
                          <th className="px-6 py-3.5 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Value</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                        {[
                          ...
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-6 py-3.5 text-right font-mono text-slate-800">{val}</td>
                          </tr>
                        ))}
                      ...
                      <table className="min-w-full divide-y divide-slate-100 text-sm">
                        <thead className="sticky top-0 z-10">
                          <tr className="bg-slate-50/95 backdrop-blur-sm">
                            <th className="px-4 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wider">SKU ID</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Gross Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">GMV</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Return Units (Amt)</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Cancel Units (Amt)</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Final Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider border-r border-slate-200">Final Amount</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Return %</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Cancel %</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Rev %</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {skuEntries.map((s, i) => (
                            <tr key={i} className="hover:bg-primary-50/30 transition-colors whitespace-nowrap">
                              <td className="px-4 py-3 font-medium text-slate-800 max-w-[200px] truncate" title={s.sku}>{s.sku}</td>
                              <td className="px-4 py-3 text-right font-mono text-slate-600">{exactStr(s.gu)}</td>
                              <td className="px-4 py-3 text-right font-mono text-slate-600">{exactStr(s.gmv)}</td>
                              ...
                              <td className="px-4 py-3 text-right font-mono font-semibold text-slate-800">{exactStr(s.fu)}</td>
                              <td className="px-4 py-3 text-right font-mono font-semibold text-slate-800 border-r border-slate-100">{exactStr(s.fa)}</td>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div className="max-h-[500px] overflow-auto">
                      <table className="min-w-full divide-y divide-slate-100 text-sm">
                        <thead className="sticky top-0 z-10">
                          <tr className="bg-slate-50/95 backdrop-blur-sm">
                            <th className="px-4 py-3 text-left font-semibold text-slate-600 text-xs uppercase tracking-wider">Category</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Gross Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider border-r border-slate-200">Revenue (GMV)</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Return Rate</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Cancel Rate</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Rev Contribution</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {catEntries.map((c, i) => (
                            <tr key={i} className="hover:bg-primary-50/30 transition-colors">
                              <td className="px-4 py-3 font-medium text-slate-800">{c.cat}</td>
                              <td className="px-4 py-3 text-right font-mono text-slate-600">{exactStr(c.gu)}</td>
                              <td className="px-4 py-3 text-right font-mono text-slate-600 border-r border-slate-100">{exactStr(c.gmv)}</td>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="flex items-center gap-4 p-5 bg-emerald-50 rounded-2xl border border-emerald-100">
                      ...
                      <div>
                        ...
                        <div className="text-lg font-bold text-slate-800">{data.maxRevDay}</div>
                        <div className="text-xs text-slate-500 font-mono">{exactStr(data.maxRevVal)}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-5 bg-slate-100 rounded-2xl border border-slate-200">
                      <div className="w-11 h-11 rounded-xl bg-slate-200 flex items-center justify-center shrink-0">
                        <CalendarDays className="w-5 h-5 text-slate-600" />
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Lowest Revenue Day</div>
                        <div className="text-lg font-bold text-slate-800">{data.minRevDay}</div>
                        <div className="text-xs text-slate-500 font-mono">{exactStr(data.minRevVal)}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-5 bg-amber-50 rounded-2xl border border-amber-100">
                      ...
                      <div>
                        ...
                        <div className="text-lg font-bold text-slate-800">{data.maxRetDay}</div>
                        <div className="text-xs text-slate-500 font-mono">{exactStr(data.maxRetVal)} units</div>
                      </div>
                    </div>
                  </div>
                      ...
                      <table className="min-w-full divide-y divide-slate-100 text-sm">
                        <thead className="sticky top-0 z-10">
                          <tr className="bg-slate-50/95 backdrop-blur-sm">
                            ...
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Gross Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">GMV</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Return Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Cancel Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Final Sale Units</th>
                            <th className="px-4 py-3 text-right font-semibold text-slate-600 text-xs uppercase tracking-wider">Final Sale Amount</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {data.sortedDates.map((d, i) => {
                            ...
                            return (
                              <tr key={i} className="hover:bg-primary-50/30 transition-colors">
                                ...
                                <td className="px-4 py-3 text-right font-mono text-slate-600">{exactStr(dt.gu)}</td>
                                <td className="px-4 py-3 text-right font-mono text-slate-600">{exactStr(dt.gmv)}</td>
                                ...
                                <td className="px-4 py-3 text-right font-mono font-semibold text-slate-800">{exactStr(dt.fu)}</td>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-800 text-lg mb-1">Revenue Concentration</h4>
                          <p className="text-slate-600 text-sm leading-relaxed">
                            <span className="font-bold text-primary-700 text-base">{data.top20Pct.toFixed(2)}%</span> of total realized revenue is generated by the Top 20% of SKUs ({data.top20Count} out of {data.totalSkuCount} items).
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Return % &gt; Overall Average ({data.globalRetAvg.toFixed(2)}%)</p>
                          </div>
                        ...
                        <div className="max-h-48 overflow-auto">
                          <table className="min-w-full divide-y divide-slate-50 text-xs">
                            <thead className="sticky top-0 bg-slate-50/95">
                              <tr>
                                <th className="text-left p-3 font-semibold text-slate-500 uppercase tracking-wider">SKU ID</th>
                                <th className="text-right p-3 font-semibold text-slate-500 uppercase tracking-wider">Return %</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                              {badRetSkus.length > 0 ? badRetSkus.map((s, i) => (
                                ...
                                <tr><td colSpan={2} className="p-4 text-center text-slate-400">No SKUs above average ({data.globalRetAvg.toFixed(2)}%)</td></tr>
                              )}
                            </tbody>
                          </table>
                        </div>
                      ...
                      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-400">Cancel % &gt; Overall Average ({data.globalCanAvg.toFixed(2)}%)</p>
                          </div>
                        ...
                        <div className="max-h-48 overflow-auto">
                          <table className="min-w-full divide-y divide-slate-50 text-xs">
                            <thead className="sticky top-0 bg-slate-50/95">
                              <tr>
                                <th className="text-left p-3 font-semibold text-slate-500 uppercase tracking-wider">SKU ID</th>
                                <th className="text-right p-3 font-semibold text-slate-500 uppercase tracking-wider">Cancel %</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                              {badCanSkus.length > 0 ? badCanSkus.map((s, i) => (
                                ...
                                <tr><td colSpan={2} className="p-4 text-center text-slate-400">No SKUs above average ({data.globalCanAvg.toFixed(2)}%)</td></tr>
                              )}
                            </tbody>
                          </table>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-800 text-lg mb-1">Revenue Dependency</h4>
                          <p className="text-slate-600 text-sm leading-relaxed">
                            The business depends on its Top 3 SKUs for <span className="font-bold text-primary-700 text-base">{data.top3Pct.toFixed(2)}%</span> of total final revenue.
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <div className="text-center py-8 border-t border-slate-200 space-y-2">
          ...
          <p className="text-xs text-slate-300">This tool is not affiliated with Flipkart. Flipkart is a trademark of Flipkart Private Limited.</p>
        </div>
      </div>
    </div>
  );
