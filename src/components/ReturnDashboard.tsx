...
function SkuDrawer({ sku, onClose }: { sku: ReturnSkuData; onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex justify-end" onClick={onClose}>
      <div
        className="w-full max-w-md bg-[#111827] h-full shadow-2xl flex flex-col animate-[modalSlideUp_0.3s_ease-out]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 border-b border-[#1E2D45] flex justify-between items-start">
          <div className="min-w-0 flex-1 mr-4">
            <h2 className="text-xl font-bold mb-1 break-all text-[#F0EDE6]">{sku.sku}</h2>
            <p className="text-sm text-[#94A3B8] line-clamp-2">{sku.product}</p>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl flex items-center justify-center text-[#475569] hover:text-[#94A3B8] hover:bg-[#1C2537] transition-all shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-[#0B0F1A] p-4 rounded-xl border border-[#1E2D45]">
              <p className="text-xs text-[#94A3B8] mb-1">Total Returns</p>
              <p className="text-2xl font-bold text-[#F0EDE6]">{sku.returns}</p>
            </div>
            ...
            <div className="bg-[#0B0F1A] p-4 rounded-xl border border-[#1E2D45]">
              <p className="text-xs text-[#94A3B8] mb-1">RTO Contribution</p>
              <p className="text-lg font-bold text-[#F0EDE6]">
                {sku.returns > 0 ? ((sku.rto / sku.returns) * 100).toFixed(1) : '0.0'}%
              </p>
            </div>
            <div className="bg-[#0B0F1A] p-4 rounded-xl border border-[#1E2D45]">
              <p className="text-xs text-[#94A3B8] mb-1">Avg Loss / Unit</p>
              <p className="text-lg font-bold text-[#F0EDE6]">
          ...
          {/* Reason Breakdown */}
          <div>
            <h4 className="font-bold mb-3 flex items-center gap-2 text-[#F0EDE6]">
              <AlertCircle className="w-4 h-4 text-[#D4A847]" />
              Return Reasons
            </h4>
                  ...
                  return (
                    <div key={reason}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-[#94A3B8] truncate pr-4">{reason}</span>
                        <span className="font-medium text-[#F0EDE6]">{count} ({percentage.toFixed(0)}%)</span>
                      </div>
          ...
          {/* Insight */}
          <div className="bg-[#1C2537] p-4 rounded-xl border border-[#A07A28]/30">
...
export function ReturnDashboard({ returnData, onUploadReturn, onClearReturn }: ReturnDashboardProps) {
  ...
  const formats = [
    ...
    { ext: '.XLSX', icon: <Sheet className="w-3.5 h-3.5" />, color: 'text-[#D4A847] bg-[#1C2537] border-primary-200' },
    { ext: '.XLS', icon: <Table2 className="w-3.5 h-3.5" />, color: 'text-amber-600 bg-amber-50 border-amber-200' },
    { ext: '.TSV', icon: <FileText className="w-3.5 h-3.5" />, color: 'text-[#94A3B8] bg-[#0B0F1A] border-[#1E2D45]' },
  ];
    ...
    return (
      <div className="space-y-6 animate-fade-in-up">
        <div className="text-center max-w-2xl mx-auto py-6">
          ...
          <h2 className="text-2xl font-bold text-[#F0EDE6] mb-2">Upload Flipkart Return Report</h2>
        ...
        <div
          ...
          className={`
            ...
            ${isDragOver
              ...
              : 'border-[#1E2D45] bg-[#111827]/80 backdrop-blur-sm hover:border-amber-300 hover:bg-amber-50/50 hover:shadow-lg'
            }
          `}
        >
          <div className={`mx-auto w-14 h-14 rounded-2xl flex items-center justify-center mb-4 transition-all duration-300 ${
            isDragOver ? 'bg-amber-100 scale-110' : 'bg-[#1C2537] group-hover:bg-amber-100 group-hover:scale-105'
          }`}>
            <Upload className={`w-7 h-7 transition-colors ${isDragOver ? 'text-amber-600' : 'text-[#475569] group-hover:text-amber-500'}`} />
          </div>

          <h3 className="text-lg font-semibold text-[#F0EDE6] mb-2">
            Drop your Flipkart Return Report here
          </h3>
          <p className="text-[#94A3B8] mb-5 text-sm">or click to browse from your computer</p>
        ...
        {/* Column format hint */}
        <div className="max-w-xl mx-auto bg-[#0B0F1A] rounded-2xl border border-[#1E2D45] p-5">
          <h4 className="font-semibold text-[#F0EDE6] text-sm mb-2 flex items-center gap-2">
    ...
    <div className="space-y-10 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-[#F0EDE6]">Return Analysis Dashboard</h2>
            <p className="text-xs text-[#475569]">{returnData.length.toLocaleString()} return records loaded</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-[#94A3B8] hover:text-rose-600 hover:bg-rose-50 border border-[#1E2D45] transition-colors"
          >
            <X className="w-4 h-4" />
      ...
      {/* Filters Bar */}
      <div className="bg-[#111827] p-4 rounded-2xl border border-[#1E2D45] flex flex-wrap gap-4 items-center justify-between shadow-sm">
        <div className="flex items-center gap-4 flex-1">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#0B0F1A] border border-[#1E2D45] rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-shadow text-sm"
            />
          </div>
          <select
            value={filters.returnType}
            onChange={(e) => setFilters(prev => ({ ...prev, returnType: e.target.value }))}
            className="px-4 py-2.5 bg-[#0B0F1A] border border-[#1E2D45] rounded-xl outline-none text-sm"
          >
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          <div className="flex items-center gap-4 p-5 bg-[#1C2537] rounded-2xl border border-[#A07A28]/30">
            ...
            <div>
              <div className="text-xs font-semibold text-[#D4A847] uppercase tracking-wider">Customer Returns</div>
              <div className="text-lg font-bold text-[#F0EDE6]">{metrics.global.customerReturnCount.toLocaleString()}</div>
              <div className="text-xs text-[#94A3B8]">{metrics.global.customerReturnRate.toFixed(1)}% of all returns</div>
            </div>
          </div>
          <div className="flex items-center gap-4 p-5 bg-amber-50 rounded-2xl border border-amber-100">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-[#F0EDE6]">{metrics.global.rtoCount.toLocaleString()}</div>
              <div className="text-xs text-[#94A3B8]">{metrics.global.rtoRate.toFixed(1)}% of all returns</div>
            </div>
          </div>
          <div className="flex items-center gap-4 p-5 bg-rose-50 rounded-2xl border border-rose-100">
            ...
            <div>
              ...
              <div className="text-lg font-bold text-[#F0EDE6]">₹{metrics.global.avgRefund.toFixed(0)}</div>
              <div className="text-xs text-[#94A3B8]">Per return record</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {metrics.insights.length > 0 && (
        <section>
          ...
          <div className="bg-gradient-to-br from-primary-50 to-indigo-50 p-6 rounded-2xl border border-[#A07A28]/30">
            ...
            <div className="space-y-3">
              {metrics.insights.map((insight, idx) => (
                <div key={idx} className="flex items-start gap-3 bg-[#111827]/70 p-4 rounded-xl border border-white">
                  ...
                  <p className="text-[#F0EDE6] font-medium text-sm leading-relaxed">{insight.text}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#0B0F1A]/95 backdrop-blur-sm">
                  <th className="px-4 py-3 text-left font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">SKU / Product</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Returns</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Refund Leakage</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">RTO Vol.</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">RTO %</th>
                  <th className="px-4 py-3 text-center font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {metrics.topSkus.map((sku) => (
                  <tr key={sku.sku} className="hover:bg-[#1C2537]/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-medium text-[#D4A847] text-sm truncate max-w-[200px]" title={sku.sku}>{sku.sku}</div>
                      <div className="text-xs text-[#94A3B8] truncate max-w-[200px]" title={sku.product}>{sku.product}</div>
                    </td>
                    <td className="px-4 py-3 text-right font-mono font-medium text-[#F0EDE6]">{sku.returns}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className="inline-flex items-center gap-1 text-xs font-medium bg-[#1C2537] hover:bg-[#1E2D45] text-[#A07A28] px-3 py-1.5 rounded-lg transition-colors"
                      >
                        Analyze
                      </button>
                    </td>
        ...
        <div className="bg-[#111827] rounded-2xl shadow-sm border border-[#1E2D45] overflow-hidden">
          <div className="max-h-[400px] overflow-auto">
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#0B0F1A]/95 backdrop-blur-sm">
                  <th className="px-4 py-3 text-left font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Reason</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Count</th>
                  <th className="px-4 py-3 text-right font-semibold text-[#94A3B8] text-xs uppercase tracking-wider">Share</th>
                  <th className="px-4 py-3 font-semibold text-[#94A3B8] text-xs uppercase tracking-wider w-1/3">Distribution</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {metrics.topReasons.map(({ reason, count }) => {
                  ...
                  return (
                    <tr key={reason} className="hover:bg-[#1C2537]/30 transition-colors">
                      <td className="px-4 py-3 font-medium text-[#F0EDE6] max-w-[250px]">
                        <span className="truncate block" title={reason}>{reason}</span>
                      </td>
                      <td className="px-4 py-3 text-right font-mono text-[#F0EDE6]">{count}</td>
                      <td className="px-4 py-3 text-right font-mono font-semibold text-[#A07A28]">{pct.toFixed(1)}%</td>
                      <td className="px-4 py-3">
                        <div className="h-2 w-full bg-[#1C2537] rounded-full overflow-hidden">
                          <div
